...
export default function Barcodes() {
  ...
  useEffect(() => { if (fetchProducts) fetchProducts() }, [fetchProducts])
  ...
  const findProduct = (code) => {
    ...
    return products.find(p => String(p.barcode) === trimmed || String(p._id) === trimmed)
  }
  ...
  const addToCart = (code) => {
    ...
    setCart(prev => {
      ...
      return [...prev, {
        ...
        sellPrice: product.sellPrice, buyPrice: product.buyPrice,
        stock: product.stock, qty: 1, total: product.sellPrice,
      }]
    })
  }

  const handleScanKeyDown = (e) => {
    if (e.key === 'Enter') { addToCart(scanInput); setScanInput('') }
  }
  ...
  const handleCheckoutConfirm = async (paymentDetails) => {
    const paymentInfo = {
      ...
      cashier: currentUser?.fullname || currentUser?.name || 'Unknown Cashier',
    ...
    if (result.success) {
      setReceipt({
        receiptId: result.sale.receiptId,  // ← fixed: use receiptId not saleId
        ...
        soldBy: result.sale.cashier || currentUser?.fullname || currentUser?.name || 'Cashier',
  ...
  const handleDownload = (product) => {
    ...
    img.onload = () => {
      ...
      ctx.font = '12px sans-serif'; ctx.fillStyle = 'var(--primary)'
    ...
    <div className={s.page}>
      ...
      {tab === 'scan' && (
        <ScanPanel
          ...
          clearCart={clearCart} setShowCheckout={setShowCheckout}
