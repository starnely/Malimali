...
| mpesa.js | **Done** | 12 sites converted across 4 authenticated routes (`POST /stk-push`, `GET /status/:checkoutRequestId`, `POST /stk-verify`, `GET /query/:checkoutRequestId`) using standard `req.tenantId` scoping, plus the special-cased `POST /callback`. The callback deliberately keeps one unscoped lookup (`Sale.findOne({mpesaCheckoutRequestId})` — a globally-unique external reference, correct as-is since there's no JWT/req.tenantId on this unauthenticated webhook), then derives `const tenantId = sale?.tenantId` and threads that derived value (not `req.tenantId`) into the one subsequent write that touches a different collection (the failure-path restock loop). Also found and tracked (not fixed): `pendingVerifications`'s in-memory `Map` state — see §9c. |
...
## 9c. Production reliability issues found during 2a-3 (independent of multi-tenancy, tracked for pre-go-live)

Not tenant-isolation bugs — found incidentally while migrating routes, worth fixing before go-live regardless of tenant count, but explicitly **not being fixed now**:

- **`mpesa.js`'s `pendingVerifications` Map is in-process, in-memory state.** It tracks split-payment M-Pesa verifications (`checkoutRequestId → {cashierId, amount}`) between `POST /stk-verify` and `POST /callback`, with entries auto-expiring after 3 minutes. If the server process restarts while a split payment is mid-flight (a deploy, a crash, a scale event), this Map is wiped — the callback would arrive, find no matching Sale *and* no matching `pendingVerifications` entry, log a warning, and silently drop the result. The cashier's split-payment M-Pesa portion would appear to hang with no resolution. This matters especially on Render (and similar PaaS), where every deploy restarts the process. Fix would be persisting this state (even a lightweight collection or the existing `Sale`/pending-transaction pattern) instead of an in-memory Map — a real but separate piece of work, not part of the tenant-scoping effort.
- **`PurchaseOrder.js`'s `generatePoNumber()` has no retry-on-collision.** Found during 2a-4 while tenant-scoping the sequence-scan query. If two POs are created concurrently for the same tenant on the same day, both can read the same "last" `poNumber` before either saves and compute the identical next sequence number — a genuine race, unchanged by the 2a-4 fix (which only added `tenantId` to the scan, not concurrency control). It fails loudly, not silently: the `{tenantId, poNumber}` unique index (2a-2) rejects the second insert with `E11000`, so no data corruption or duplicate PO numbers can land — the second request's create simply errors out (500) and the user must retry. Fix would be a regenerate-and-retry loop around the save (or an atomic counter/sequence document per tenant) instead of read-then-write — a real but separate piece of work, not part of the tenant-scoping effort.
