    ...
    for (const recv of receivedItems) {
      ...
      if (product) {
        ...
        if (product.stock <= (product.reorderLevel ?? 5))
          lowStockAlerts.push({ productId: product._id, productName: product.name, stock: product.stock, reorderLevel: product.reorderLevel, store: product.store })
      }
    }
...
// ── 8. RECORD INVOICE ─────────────────────────────────────────────────
// Called when supplier delivers and hands over their invoice
...
router.get("/:id/pdf", async (req, res) => {
  ...
  try {
    ...
    let cx = LEFT + 6
    ;["#", "DESCRIPTION", "QTY", "UNIT COST", "LINE TOTAL"].forEach((h, i) => {
    ...
    po.items.forEach((item, i) => {
      ...
      doc.fillColor(C.text).font("Helvetica").text(String(item.qtyOrdered), rx, y + 7, { width: COL.qty, align: "center" }); rx += COL.qty
      doc.text(`KSh ${Number(item.unitCost).toLocaleString()}`, rx, y + 7, { width: COL.unit, align: "right" }); rx += COL.unit
    ...
    // Invoice & Payment section on PDF
  ...
  try {
    ...
    const itemsHtml = po.items.map((item, i) => `
      <tr style="background:${i % 2 === 0 ? "#ffffff" : "#F5F7FF"}">
        ...
        <td style="padding:10px 14px;border-bottom:1px solid #E2E8F0;text-align:center;color:#475569">${item.qtyOrdered}</td>
        <td style="padding:10px 14px;border-bottom:1px solid #E2E8F0;text-align:right;color:#475569">KSh ${Number(item.unitCost).toLocaleString()}</td>
...
// GET /api/purchase-orders/supplier-payments?from=YYYY-MM-DD&to=YYYY-MM-DD
